...
# ========================= CONSTANTS ============================
AFKSTR = [
    f"**Maaf Gays {ALIVE_NAME} Sedang AFK!**",
    ...
    if mention.message.mentioned and not (await mention.get_sender()).bot:
        if ISAFK:
            ...
            if mention.sender_id not in USERS:
                if AFKREASON:
                    await mention.reply(f"**✘ Maaf Gays Si Nganu {ALIVE_NAME} Sedang AFK** {afk_since} **Yang Lalu.**\
                        \n☛ **Alasan:** `{AFKREASON}`")
                else:
                ...
                if USERS[mention.sender_id] % randint(2, 4) == 0:
                    if AFKREASON:
                        await mention.reply(f"**✘ Maaf Si Nganu Masih AFK** {afk_since} **Yang Lalu.**\
                            \n☛ **Alasan:** `{AFKREASON}`")
                    else:
                        await mention.reply(str(choice(AFKSTR)))
    ...
    if sender.is_private and sender.sender_id != 777000 and not (
        ...
        if apprv and ISAFK:
            ...
            if sender.sender_id not in USERS:
                if AFKREASON:
                    await sender.reply(f"✘ **Maap Si Nganu Sedang AFK** {afk_since} **Yang Lalu**.\
                        \n☛ **Alasan**: `{AFKREASON}`")
                else:
